...
class CountryFlagMatchingQuestionRepository {
    delete(questionId) {
    ...
    retrieveById(questionId) {
        let query = "SELECT q.id, q.questionText, q.correctAnswer,q.timeLimit, q.points, mq.flagOptions " +
    ...
    retrieveRandom() {
        let query = "SELECT * FROM CountryFlagMatchingQuestion mq JOIN question q where mq.id=q.id ORDER BY RAND() LIMIT 1";
        return new Promise((resolve, reject) => {
        ...
        return new Promise((resolve, reject) => {
            db_1.default.query("INSERT INTO Question (id, questionText, correctAnswer, timeLimit, points) VALUES(?,?,?,?,?)", [question.id, question.questionText, question.correctAnswer, question.timeLimit, question.points], (err, res) => {
                if (err) {
                    ...
                    db_1.default.query("INSERT INTO CountryFlagMatchingQuestion (id,flagOptions) VALUES (?,?)", [question.id, JSON.stringify(question.flagOptions)], (err, res) => {
                        if (err) {
...
exports.default = new CountryFlagMatchingQuestionRepository();
